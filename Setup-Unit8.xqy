...
let $config := <forest-properties xmlns="http://marklogic.com/manage">
  <forest-name>sec-unit-08</forest-name>
 </forest-properties>
...
return if ($response//*:code/string() = "201") then
  $config//*:name || "sec-unit-08 forest created."
else
  $response
...
let $config := <database-properties xmlns="http://marklogic.com/manage">
  <database-name>sec-unit-08</database-name>
  <forests>
    <forest>sec-unit-08</forest>
  </forests>
...
return if ($response//*:code/string() = "201") then
  $config//*:name || "sec-unit-8 database created."
else
  $response
